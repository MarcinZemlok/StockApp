require 'dotenv/load'
require 'httparty'

$finnhubToken = ENV['FINNHUB_KEY']
$quandlToken = ENV['QUANDL_KEY']

class IndexController < ApplicationController
    def index
        # @symbols = fetchSymbols

        # @trackSymbols = {
        #     'ENT.WA'=>{},
        #     'RBW.WA'=>{}
        # }
        # fetchQuote

        # @recommendations = {
        #     'ENT.WA'=>{},
        #     'RBW.WA'=>{}
        # }
        # fetchRecommendations

        @qData = {}
        @filteredIndexes = $allIndexes[0..1]
        fetchQuandle

        for i in (0...@filteredIndexes.length)
            for j in (0...@filteredIndexes.length)
                if(@qData[@filteredIndexes[i]]['diff'] < @qData[@filteredIndexes[j]]['diff'])
                    @filteredIndexes[i], @filteredIndexes[j] = @filteredIndexes[j], @filteredIndexes[i]
                end
            end
        end
    end

    def fetchQuandle
        @filteredIndexes.each do |symbol|
            url = "https://www.quandl.com/api/v3/datasets/WSE/#{symbol}?api_key=#{$quandlToken}&column_index=1&limit=50"

            res = HTTParty.get(url)

            puts "Quandle fetch: #{res.code}"

            @qData[symbol] = JSON.parse(res.body)

            @qData[symbol]['diff'] = @qData[symbol]['dataset']['data'][0][1] - @qData[symbol]['dataset']['data'][-1][1]
        end
    end

    def fetchSymbols()
        res = HTTParty.get("https://finnhub.io/api/v1/stock/symbol?exchange=WA&token=#{$finnhubToken}")

        puts "Symbols fetch: #{res.code}"

        return JSON.parse(res.body)
    end

    def fetchQuote()
        @trackSymbols.keys.each do |symbol|
            s = symbol[0..2]
            res = HTTParty.get("https://finnhub.io/api/v1/quote?symbol=#{s}&token=#{$finnhubToken}")

            puts "Quotes fetch: #{res.code}"

            @trackSymbols[symbol] = JSON.parse(res.body)
        end
    end

    def fetchRecommendations()
        @trackSymbols.keys.each do |symbol|
            s = symbol[0..2]
            res = HTTParty.get("https://finnhub.io/api/v1/stock/recommendation?symbol=#{s}&token=#{$finnhubToken}")

            puts "Recommendations fetch: #{res.code}"

            @recommendations[symbol] = JSON.parse(res.body)
        end
    end
end

# All Polish stox inex codes
$allIndexes = [
    "01NFI",
    "04PRO",
    "05VICT",
    "06MAGNA",
    "08OCTAVA",
    "10FOKSAL",
    "11BIT",
    "11_NFI",
    "13FORTUNA",
    "14ZACH",
    "4FUNMEDIA",
    "4MEDIA",
    "AATHOLD",
    "ABADONRE",
    "ABCDATA",
    "ABG",
    "ABMSOLID",
    "ABPL",
    "ACAUTOGAZ",
    "ACE",
    "ACTION",
    "ADIUVO",
    "ADVADIS",
    "ADVGRUPA",
    "AGORA",
    "AGROS",
    "AGROTON",
    "AGROWILL",
    "AILLERON",
    "AIRWAY",
    "ALCHEMIA",
    "ALIOR",
    "ALMA",
    "ALTA",
    "ALTERCO",
    "ALTUSTFI",
    "ALUMETAL",
    "AMBRA",
    "AMICA",
    "AMPLI",
    "AMREST",
    "AMS",
    "ANIMEX",
    "ANTI",
    "APATOR",
    "APEXIM",
    "APLISENS",
    "APLITT",
    "APSENERGY",
    "ARCHICOM",
    "ARCTIC",
    "ARCUS",
    "ARMATURA",
    "ARTERIA",
    "ARTIFEX",
    "ARTMAN",
    "ASBIS",
    "ASMGROUP",
    "ASSECOBS",
    "ASSECOPOL",
    "ASSECOSEE",
    "ASSECOSLO",
    "ASTARTA",
    "ATAL",
    "ATENDE",
    "ATLANTAPL",
    "ATLANTIS",
    "ATLASEST",
    "ATM",
    "ATMGRUPA",
    "ATMSI",
    "ATREM",
    "AUGA",
    "AURUM",
    "AUTOPARTN",
    "AVIAAML",
    "AVIASG",
    "AWBUD",
    "AZOTYTARNOW",
    "B3SYSTEM",
    "BACA",
    "BACD",
    "BAHOLDING",
    "BAKALLAND",
    "BAKOMA",
    "BALTONA",
    "BANKBPH",
    "BANKIER_PL",
    "BARLINEK",
    "BBICAPNFI",
    "BBIDEV",
    "BBIDEVNFI",
    "BBIZEN",
    "BBIZENNFI",
    "BCHEM",
    "BCZ",
    "BEDZIN",
    "BEEFSAN",
    "BELCHATOW",
    "BENEFIT",
    "BERLING",
    "BEST",
    "BETACOM",
    "BETONSTAL",
    "BGDA",
    "BGDA_SKI",
    "BGZ",
    "BGZBNPP",
    "BICK",
    "BIELBAW",
    "BIG2",
    "BIG_2",
    "BIK",
    "BIOMEDLUB",
    "BIOTON",
    "BIPROMET",
    "BLACKLION",
    "BMPAG",
    "BNPPL",
    "BNPPPL",
    "BOGDANKA",
    "BOMI",
    "BOOMBIT",
    "BORYSZEW",
    "BOS",
    "BOWIM",
    "BRASTER",
    "BRE",
    "BRIJU",
    "BROK",
    "BSCDRUK",
    "BUDIMEX",
    "BUDIMEX_POZNA_",
    "BUDOPOL",
    "BUDVARCEN",
    "BUMECH",
    "BUWOG",
    "BYTOM",
    "BZWBK",
    "BZWBK2",
    "CALATRAVA",
    "CAMMEDIA",
    "CAPITAL",
    "CASHFLOW",
    "CCC",
    "CCENERGY",
    "CCIINT",
    "CDPROJEKT",
    "CDRED",
    "CDRL",
    "CEDC",
    # "CEEPLUS",
    "CELTIC",
    "CENSTALGD",
    "CENTKLIMA",
    "CENTROZAP",
    "CERSANIT",
    "CEZ",
    "CFI",
    "CHEMOS",
    "CIECH",
    "CIGAMES",
    "CITYINTER",
    "CITYSERV",
    "CLIF",
    "CLNPHARMA",
    "CNT",
    "COALENERG",
    "COGNOR",
    "COLIAN",
    "COMARCH",
    "COMP",
    "COMPENSA",
    "COMPERIA",
    "COMPLEX",
    "CORMAY",
    "CPENERGIA",
    "CPGROUP",
    "CSS",
    "CUBEITG",
    "CYFRPLSAT",
    "CZTOREBKA",
    "DATAWALK",
    "DBPBC",
    "DEBICA",
    "DECORA",
    "DEKPOL",
    "DELIA",
    "DELKO",
    "DEVELIA",
    "DGA",
    "DIGITREE",
    "DINOPL",
    "DMWDM",
    "DOMDEV",
    "DOMPLAST",
    "DRAGOWSKI",
    "DREWEX",
    "DROP",
    "DROSED",
    "DROZAPOL",
    "DSS",
    "DTP",
    "DUDA",
    "DUON",
    "DZPOLSKA",
    "EBI",
    "ECARD",
    "ECHO",
    "EDINVEST",
    "EFEKT",
    "EFH",
    "EFL",
    "EKANCELAR",
    "EKO",
    "EKODROB",
    "EKOEXPORT",
    "ELBUDOWA",
    "ELEKTRIM",
    "ELEKTROEX",
    "ELEKTROTI",
    "ELEMENTAL",
    "ELKOP",
    "ELSTAROIL",
    "ELZAB",
    "EMAX",
    "EMCINSMED",
    "EMPERIA",
    "ENAP",
    "ENEA",
    "ENELMED",
    "ENERGA",
    "ENERGOINS",
    "ENERGOPLD",
    "ENERGOPN",
    "ENERGOPOL",
    "ENTER",
    "ERBUD",
    "ERG",
    "ERGIS",
    "ESOTIQ",
    "ESPEBEPE",
    "ESSYSTEM",
    "ESTAR",
    "EUCO",
    "EUIMPLANT",
    "EUROCASH",
    "EUROFAKTR",
    "EUROHOLD",
    "EUROMARK",
    "EUROTEL",
    "EVEREST",
    "EXBUD",
    "EXBUD_2",
    "EXILLON",
    "FAM",
    "FAMOT",
    "FAMUR",
    "FARMACOL",
    "FARMFOOD",
    "FASING",
    "FASTFIN",
    "FEERUM",
    "FENGHUA",
    "FERRO",
    "FERRUM",
    "FMG",
    "FON",
    "FORTE",
    "FORTISPL",
    "FORTUNA",
    "FORTWRO",
    "FOTA",
    "GADUGADU",
    "GANT",
    "GASTELZUR",
    "GEKOPLAST",
    "GETBACK",
    "GETBANK",
    "GETIN",
    "GETINOBLE",
    "GFPREMIUM",
    "GINOROSSI",
    "GKI",
    "GLCOSMED",
    "GLOBALNGR",
    "GLOBALNRG",
    "GLOBCITYHD",
    "GOBARTO",
    "GORENJE",
    "GPRD",
    "GPRE",
    "GPW",
    "GRAAL",
    "G_RA_D_E",
    "GRAJEWO",
    "GRAVITON",
    "GREENECO",
    "GREMINWES",
    "GREMISLTN",
    "GREMMEDIA",
    "GROCLIN",
    "GRODNO",
    "GRUPAAZOTY",
    "GRUPAONET",
    "GTC",
    "HANDLOWY",
    "HARDEX",
    "HARPER",
    "HAWE",
    "HBPOLSKA",
    "HBWLOCLAW",
    "HELIO",
    "HERKULES",
    "HERMAN",
    "HMINWEST",
    "HOLLYWOOD",
    "HOOP",
    "HOWELL",
    "HTLSTREFA",
    "HUBSTYLE",
    "HUTMEN",
    "HYDROBUD",
    "HYDROGD",
    "HYDROTOR",
    "HYGIENIKA",
    "HYPERION",
    "I2DEV",
    "IALBGR",
    "IBSM",
    "IBSYSTEM",
    "ICOPAL",
    "IDEABANK",
    "IDEATFI",
    "IDEON",
    "IDMSA",
    "IFCAPITAL",
    "IFIRMA",
    "IFSA",
    "IGROUP",
    "IIAAV",
    "IMCOMPANY",
    "IMMOBILE",
    "IMMOEAST",
    "IMPEL",
    "IMPERA",
    "IMPEXMET",
    "IMS",
    "INC",
    "INDATA",
    "INDYGO",
    "INDYKPOL",
    "INGBSK",
    "INPOST",
    "INPRO",
    "INSTAL",
    "INSTALKRK",
    "INTAKUS",
    "INTEGERPL",
    "INTERAOLT",
    "INTERBUD",
    "INTERCARS",
    "INTERFERI",
    "INTERIA_PL",
    "INTERSPPL",
    "INTROL",
    "INVENTUM",
    "INVESTCON",
    "INVESTORMS",
    "INVISTA",
    "INWESTCON",
    "IPOPEMA",
    "IQP",
    "IRENA",
    "ITMTRADE",
    "IVAXCORP",
    "IVMX",
    "IZNS",
    "IZOBLOK",
    "IZOLACJA",
    "IZOSTAL",
    "JAGO",
    "JAROS_AW",
    "JCAUTO",
    "JELFA",
    "JHMDEV",
    "JJAUTO",
    "JSW",
    "JUPITER",
    "JUTRZENKA",
    "JWCONSTR",
    "JWWINVEST",
    "K2INTERNT",
    "KABLE",
    "KABLEHOLD",
    "KANIA",
    "KAREN",
    "KBDOM",
    "KCI",
    "KDMSHIPNG",
    "KERDOS",
    "KERNEL",
    "KETY",
    "KGHM",
    "KGL",
    "KINOPOL",
    "KOELNER",
    "KOFOL",
    "KOFOLA",
    "KOGENERA",
    "KOLASTYNA",
    "KOMPAP",
    "KOMPUTRON",
    "KONSSTALI",
    "KO_O",
    "KOPEX",
    "KOV",
    "KPPD",
    "KRAKBROK",
    "KRAKCHEM",
    "KREC",
    "KREDYTB",
    "KREDYTIN",
    "KREZUS",
    "KRKA",
    "KROSNO",
    "KRUK",
    "KRUSZWICA",
    "KRVITAMIN",
    "KSGAGRO",
    "KZWM",
    "LABOPRINT",
    "LARK",
    "LARQ",
    "LCCORP",
    "LDASA",
    "LENA",
    "LENTEX",
    "LETA",
    "LGPETRO",
    "LIBET",
    "LIBRA",
    "LIVECHAT",
    "LOKUM",
    "LOTOS",
    "LPP",
    "LSISOFT",
    "LSTCAPITA",
    "LTL",
    "LUBAWA",
    "LUKBUT",
    "LZPS",
    "MABION",
    "MAGELLAN",
    "MAKARONPL",
    "MAKRUM",
    "MANGATA",
    "MANOMETRY",
    "MARVIPOL",
    "MASTERPHA",
    "MASTERS",
    "MAXCOM",
    "MBANK",
    "MBWS",
    "MCI",
    "MCLOGIC",
    "MDIENERGIA",
    "MEDIACAP",
    "MEDIATEL",
    "MEDICALG",
    "MEGARON",
    "MENNICA",
    "MERCATOR",
    "MERCOR",
    "MEWA",
    "MEXPOLSKA",
    "MFO",
    "MIDAS",
    "MIESZKO",
    "MILKILAND",
    "MILLENNIUM",
    "MILMET",
    "MIRACULUM",
    "MIRBUD",
    "MISPOL",
    "MIT",
    "MITEX",
    "MLPGROUP",
    "MLSYSTEM",
    "MMPPL",
    "MNI",
    "MOBRUK",
    "MOJ",
    "MOL",
    "MONNARI",
    "MORIZON",
    "MORLINY",
    "MOSTALEXP",
    "MOSTALGD",
    "MOSTALPLC",
    "MOSTALWAR",
    "MOSTALZAB",
    "MOSTOSTAL_KRAK",
    "MOSTOSTAL_KRAK_W",
    "MSXRESOUR",
    "MUZA",
    "MWIG40",
    "MWIG40DVP",
    "MWIG40TR",
    "MWTRADE",
    "NAFTA",
    "NANOGROUP",
    "NCINDEX",
    "NEPENTES",
    "NETIA",
    "NETIA2",
    "NETMEDIA",
    "NEUCA",
    "NEWAG",
    "NEWWORLDN",
    "NEWWORLDR",
    "NFIEMF",
    "NOBLEBANK",
    "NOKAUT",
    "NOMI",
    "NORDEABP",
    "NORTCOAST",
    "NOVAKBM",
    "NOVATURAS",
    "NOVITA",
    "NOVITUS",
    "NOWAGALA",
    "NTTSYSTEM",
    "OAT",
    "OBORNIKI",
    "OCEAN",
    "ODLEWNIE",
    "OEX",
    "OKOCIM",
    "OKOCIM_2",
    "OLAWA",
    "OLYMPIC",
    "ONE2ONE",
    "OPENFIN",
    "OPOCZNO",
    "OPONEO",
    "OPONEO_PL",
    "OPTEAM",
    "OPTIMUS",
    "OPTIMUS_2",
    "OPTOPOL",
    "ORANGEPL",
    "ORBIS",
    "ORCOGROUP",
    "ORFE",
    "ORION",
    "ORZBIALY",
    "ORZEL",
    "OTLOG",
    "OTMUCHOW",
    "OVOSTAR",
    "PAGED",
    "PAMAPOL",
    "PANOVA",
    "PATENTUS",
    "PAZUR",
    "PBG",
    "PBK",
    "PBKM",
    "PBOANIOLA",
    "PBR",
    "PBSFINANSE",
    "PCCEXOL",
    "PCCINTER",
    "PCCROKITA",
    "PCGUARD",
    "PCM",
    "PEGAS",
    "PEIXIN",
    "PEKABEX",
    "PEKAES",
    "PEKAO",
    "PELION",
    "PEMANAGER",
    "PEMUG",
    "PEP",
    "PEPEES",
    "PERMEDIA",
    "PETROLINV",
    "PFLEIDER",
    "PGE",
    "PGF",
    "PGNIG",
    "PGO",
    "PGODLEW",
    "PGSSOFT",
    "PHARMENA",
    "PHN",
    "PIASECKI",
    "PKNORLEN",
    "PKOBP",
    "PKPCARGO",
    "PLASTBOX",
    "PLATYNINW",
    "PLAY",
    "PLAYWAY",
    "PLAZACNTR",
    "PMPG",
    "POINTGROUP",
    "POLAQUA",
    "POLAR",
    "POLCOLOR",
    "POLCOLORIT",
    "POLFKUTNO",
    "POLICE",
    "POLIFARBC",
    "POLIFARB_D_BICA",
    "POLIFARBW",
    "POLIFC2",
    "POLIGR",
    "POLIMEXMS",
    "POLISA",
    "POLJADLO",
    "POLLENAE",
    "POLMED",
    "POLMOSBIA",
    "POLMOSLBN",
    "POLNA",
    "POLNORD",
    "POLREST",
    "POLWAX",
    "POLYMETAL",
    "PONAR",
    "PONARFEH",
    "POZBUD",
    "POZMEAT",
    "PPG",
    "PPLHOLD",
    "PPWK",
    "PRAGMAFA",
    "PRAGMAINK",
    "PRAIRIE",
    "PRATERM",
    "PRESCO",
    "PRIMAMODA",
    "PRIMETECH",
    "PROCAD",
    "PROCHEM",
    "PROCHNIK",
    "PROJPRZEM",
    "PROKOM",
    "PRONOX",
    "PROSPER",
    "PROTEKTOR",
    "PROVIDENT",
    "PROVIMROL",
    "PSG",
    "PTI",
    "PULAWY",
    "PWRMEDIA",
    "PZU",
    "QUANTUM",
    "QUERCUS",
    "QUMAK",
    "QUMAKSEK",
    "R22",
    "RADPOL",
    "RAFAKO",
    "RAFAMET",
    "RAINBOW",
    "RANKPROGR",
    "RAWLPLUG",
    "REDAN",
    "REDWOOD",
    "REGNON",
    "REINHOLD",
    "REINO",
    "RELPOL",
    "REMAK",
    "RESBUD",
    "RESPECT",
    "RMFFM",
    "ROBYG",
    "RONSON",
    "ROPCZYCE",
    "ROVESE",
    "RUBICON",
    "RUCH",
    "RYVU",
    "SADOVAYA",
    "SANOK",
    "SANPL",
    "SANPL2",
    "SANTANDER",
    "SANWIL",
    "SARE",
    "SCOPAK",
    "SECOGROUP",
    "SEKO",
    "SELENAFM",
    "SELVITA",
    "SERINUS",
    "SESCOM",
    "SETANTA",
    "SFINKS",
    "SILVAIR_REGS",
    "SILVANO",
    "SIMPLE",
    "SKARBIEC",
    "SKOK",
    "SKOTAN",
    "SKYEUROPE",
    "SKYLINE",
    "SKYSTONE",
    "SLEEPZAG",
    "SMT",
    "SNIEZKA",
    "SOBIESKI",
    "SOHODEV",
    "SOKOLOW",
    "SOLAR",
    "SONEL",
    "SOPHARMA",
    "SPIN",
    "SPRAY",
    "SRUBEX",
    "STALEXP",
    "STALEXP2",
    "STALPROD",
    "STALPROFI",
    "STAPORKOW",
    "STARHEDGE",
    "STELMET",
    "STGROUP",
    "STOMIL",
    "STORMM",
    "SUNEX",
    "SUWARY",
    "SWARZEDZ",
    "SWIECIE",
    "SWIG80",
    "SWIG80DVP",
    "SWIG80TR",
    "SWISSMED",
    "SYGNITY",
    "SYNEKTIK",
    "SYNTHOS",
    "TALANX",
    "TALEX",
    "TARCZYNSKI",
    "TATRY",
    "TAURONPE",
    "TBSP_INDEX",
    "TBULL",
    "TECHMEX",
    "TELEPOLSKA",
    "TELL",
    "TERESA",
    "TERMOREX",
    "TESGAS",
    "TETA",
    "TFONE",
    "TIM",
    "TONSIL",
    "TOORA",
    "TOPMEDICA",
    "TORFARM",
    "TORPOL",
    "TOWERINVT",
    "TOYA",
    "TPSA",
    "TRAKCJA",
    "TRANSPOL",
    "TRASINTUR",
    "TRAVELPL",
    "TRION",
    "TRITON",
    "TSGAMES",
    "TUEUROPA",
    "TUP",
    "TVN",
    "TXM",
    "ULMA",
    "ULTGAMES",
    "UNIBEP",
    "UNIBUD",
    "UNICREDIT",
    "UNIMA",
    "UNIMIL",
    "UNIMOT",
    "UNIVERSAL",
    "UNIWHEELS",
    "URSUS",
    "VANTAGE",
    "VARIANT",
    "VENTUREIN",
    "VIGOSYS",
    "VINDEXUS",
    "VISCO",
    "VISTAL",
    "VISTULA",
    "VIVID",
    "VOTUM",
    "VOXEL",
    "VRG",
    "WADEX",
    "WAFAPOMP",
    "WANDALEX",
    "WARFAMA",
    "WARIMPEX",
    "WARTA",
    "WASKO",
    "WAWEL",
    "WBK",
    "WDMCP",
    "WDX",
    "WEDEL",
    "WESTAISIC",
    "WIELTON",
    "WIG",
    "WIG20",
    "WIG20___",
    "WIG20DVP",
    "WIG20LEV",
    "WIG20SHORT",
    "WIG20TR",
    "WIG30",
    "WIG30TR",
    "_WIG40___",
    "_WIG80___",
    "WIG_BANKI",
    "WIG_BUDOW",
    "WIG_CEE",
    "WIG_CHEMIA",
    "WIG_DEWEL",
    "WIGDIV",
    "WIG_ENERG",
    "WIG_ESG",
    "WIG_GAMES",
    "WIG_GORNIC",
    "WIG_INFO",
    "WIG_LEKI",
    "WIG_MEDIA",
    "WIG_MOTO",
    "WIG_MS_BAS",
    "WIG_MS_FIN",
    "WIG_MS_PET",
    "WIG_NRCHOM",
    "WIG_ODZIEZ",
    "WIG_PALIWA",
    "WIG_POLAND",
    "WIG_SPOZYW",
    "WIG_SUROWC",
    "WIGTECH",
    "WIG_TELKOM",
    "WIG_UKRAIN",
    "WIKANA",
    "WILBO",
    "WINDMOBIL",
    "WINVEST",
    "WIRTUALNA",
    "WISTIL",
    "WITTCHEN",
    "WKSM",
    "WOJAS",
    "WOLAINFO",
    "WOLCZANKA",
    "WORKSERV",
    "WSIP",
    "XTB",
    "XTPL",
    "XTRADEBDM",
    "YAWAL",
    "YOLO",
    "ZAMET",
    "ZASADA",
    "ZASTAL",
    "ZEG",
    "ZELMER",
    "ZEPAK",
    "ZETKAMA",
    "ZEW",
    "ZNTKLAPY",
    "ZPUE",
    "ZREMB",
    "ZREW",
    "ZTSERG",
    "ZUE",
    "ZURAWIE",
    "ZYWIEC"
]
